...
- **No dark mode is documented** (see Known Gaps). The app leans on a light `{colors.canvas}`
  ...
  border and pill actions — NOT a translucent dark Spotlight panel. If a dark theme is wanted,
  it must be added to this system (closest existing anchor: `color-block-section-navy` +
  inverse-canvas). Decide in /design-consultation.
- **Notch panel** is tiny (~640px): use `{typography.body}`/`{typography.body-lg}` and
  `{typography.card-title}`, NOT `display-xl` (86px headlines are for the full window /
  onboarding only). Preset actions = `{rounded.pill}` buttons; selected/active = primary fill.
- **Notebook full window**: `{colors.block-*}` color-block sections become the section/grouping
  device (e.g. date groups, the answer surface); pricing/marketing components (pricing tabs,
  marquee, comparison glyphs) are not used in-app.
- **Type**: figmaSans → Inter, figmaMono → JetBrains Mono (per Font Substitutes). Weight, not
  opacity, carries hierarchy.
...
> Source pages: /design/, /figjam/brainstorming-tool/, /pricing/, /contact/.
...
- **figmaSans** — Figma's proprietary variable typeface; fallback stack `figmaSans Fallback, SF Pro Display, system-ui, helvetica`. Variable weight axis is exercised at unusually fine increments (320, 340, 450, 480, 540, 700) — the design system reads as a single voice modulating rather than a stepped weight family.
...
| `{typography.body-lg}` | 20px | 330 | 1.40 | -0.14px | Lead body on hero, contact form labels |
...
- **Mono is taxonomy, not body.** figmaMono reserved for eyebrows and captions — never used to set a paragraph.
...
- Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.button}`, padding 8px 20px, rounded `{rounded.pill}`.
...
- Selected: `{colors.primary}` background, `{colors.on-primary}` text — exactly the same surface as `button-primary`, which makes the selected tab read like an active CTA, not a passive state.
...
**`color-block-section-navy`** — deep indigo ground for the home "Ship products" story; the only inverse color-block surface above the footer.
...
- When introducing a story section, choose **one** color block from the `{colors.block-*}` and let it span full content width with `{rounded.lg}` corners and `{spacing.xxl}` interior padding.
...
- **Color-block sections**: above 768px the section keeps `{spacing.xl}` of canvas around it so the rounded corners read; below 768px the corners are removed and the block bleeds to viewport edge for a poster effect.
